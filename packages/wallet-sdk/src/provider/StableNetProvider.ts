...
export class StableNetProvider {
  ...
  async reconnect(): Promise<Address[]> {
    const accounts = await this.connectionManager.reconnect()
  ...
  /** Adopt an already authorized EIP-1193 connection without opening a prompt. */
  async syncConnection(): Promise<Address[]> {
    const accounts = await this.getAccounts()
    if (accounts.length > 0) await this.chainManager.getChainId()
    else this.chainManager.reset()
    this.connectionManager.handleAccountsChanged(accounts)
    return accounts
  }
