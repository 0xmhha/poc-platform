...
func IsValidSlippage(slippage float64) bool {
	return slippage >= 0 && slippage <= 10000
}
...
// ValidateAmountOrZero validates an amount field (must be non-negative integer string)
func ValidateAmountOrZero(amount, fieldName string) *ValidationError {
	if amount == "" {
		return &ValidationError{
			Field:   fieldName,
			Message: "is required",
...
// ValidateSlippage validates a slippage value
func ValidateSlippage(slippage float64) *ValidationError {
...
// ValidateAmountOrZero validates and adds error if invalid
func (v *Validator) ValidateAmountOrZero(amount, fieldName string) *Validator {
	v.AddError(ValidateAmountOrZero(amount, fieldName))
	return v
}
...
// ValidateSlippage validates and adds error if invalid
func (v *Validator) ValidateSlippage(slippage float64) *Validator {
	v.AddError(ValidateSlippage(slippage))
	return v
}
