...
function postOp(
    ...
    // v0.7+: postOpReverted 모드는 EntryPoint가 내부적으로 처리하며,
    // Paymaster에 이 모드로 호출하지 않음. mode는 opSucceeded 또는 opReverted만 전달됨.
...
sequenceDiagram
    ...
    Bundler->>EntryPoint: 7. handleOps (view/trace call 시뮬레이션)
...
번들러는 다음을 수행합니다:
1. 기본 형식 검증
2. `handleOps()` view/trace call로 검증 시뮬레이션 (v0.7+에서 `simulateValidation()` 제거됨)
3. 멤풀에 추가
...
2. **Paymaster 후처리** (있는 경우)
   ```solidity
   IPaymaster(paymaster).postOp(PostOpMode.opSucceeded, context, actualGasCost, gasPrice);
   ```
...
flowchart TD
    ...
    DomainSep --> DomainHash[domainSeparator =<br/>keccak256 EIP712Domain<br/>name: ERC4337<br/>version: 1<br/>chainId: chainId<br/>verifyingContract: EntryPoint]
...
- [ ] UserOperation 멤풀 관리
- [ ] `handleOps()` view/trace call로 시뮬레이션 (v0.7+)
- [ ] 번들링 로직
- [ ] 트랜잭션 제출
- [ ] 수수료 최적화
