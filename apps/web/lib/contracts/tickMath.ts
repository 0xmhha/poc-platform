...
// Port of Uniswap v3-core TickMath.getSqrtRatioAtTick, shipped in poc-contract/lib/v3-core.
const factors = [
...
export function sqrtRatioAtTick(tick: number): bigint {
  if (!Number.isInteger(tick) || Math.abs(tick) > 887272) throw Error('Tick out of range')
  const abs = Math.abs(tick)
  let ratio = 1n << 128n
  for (let i = 0; i < factors.length; i++) if (abs & (1 << i)) ratio = (ratio * factors[i]) >> 128n
