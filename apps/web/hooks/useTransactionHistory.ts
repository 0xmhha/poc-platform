...
export function useTransactionHistory(
  ...
  const { publicClient, chainId, indexerUrl } = useStableNetContext()
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      if (externalFetch) {
        ...
        // The indexer includes native transfers, failed calls, and older history.
        // Transfer logs enrich those transactions and cover recent token activity.
        const nativeResult = await createIndexerClient(indexerUrl)
          .getTransactionsByAddress(address, 50)
          .catch(() => null)
        if (id !== fetchIdRef.current) return
        if (!nativeResult)
          setError(new Error('Indexer unavailable: transaction history is incomplete'))
        ...
        const tokenByHash = new Map(mapped.map((tx) => [tx.hash.toLowerCase(), tx.tokenTransfer]))
        const nativeTransactions: Transaction[] = (nativeResult?.nodes ?? []).map((tx) => ({
          hash: tx.hash as Hex,
          ...
          tokenTransfer: tokenByHash.get(tx.hash.toLowerCase()),
        }))
        const nativeHashes = new Set(nativeTransactions.map((tx) => tx.hash.toLowerCase()))
        const merged = [
          ...nativeTransactions,
          ...mapped.filter((tx) => !nativeHashes.has(tx.hash.toLowerCase())),
        ]
  ...
  }, [address, externalFetch, publicClient, chainId, indexerUrl])
