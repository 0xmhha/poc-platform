...
export function useStableNetWallet(): UseStableNetWalletResult {
  ...
  useEffect(() => {
    ...
    let disposed = false
    let wrapper: StableNetProvider | undefined
    setProvider(null)
    connector
      .getProvider()
      .then(async (raw) => {
        if (!raw || disposed) return
        wrapper = new StableNetProvider(raw as EIP1193Provider)
        await wrapper.syncConnection()
        if (!disposed) setProvider(wrapper)
      })
      .catch(() => {
        wrapper?.destroy()
        if (!disposed) setProvider(null)
      })
    return () => {
      disposed = true
      wrapper?.destroy()
